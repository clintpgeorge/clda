...
List clda_mgs(
  ...
  double doc_denom;
  double step_size_sqrd = step_size * step_size;
  ...
  arma::mat psi_t; // K x J matrix
  arma::mat psi_grad; // K x J matrix

  arma::mat psi_prop; // K x J matrix
  arma::mat psi_prop_grad; // K x J matrix

  arma::mat Epsilon; // K x J matrix
  arma::mat PSI_GRAD_INIT(num_topics, num_collections); // K x J matrix
  PSI_GRAD_INIT.fill(alpha_h - 1.);
  ...
  cout << endl << endl;
  if (verbose > 1){
    cout << "clda-mgs (c++): Number of saved samples - "
         << valid_samples << endl;
  }
  // Calculates the document word indices

  if (verbose > 0){
    cout << "clda-mgs (c++): Initializes variables and count statistics....";
  }
  ...
  if (verbose > 1){
    cout << "clda-mgs (c++): Number of collections: " << num_collections << endl;
    cout << "clda-mgs (c++): Number of docs: " << num_docs << endl;
    cout << "clda-mgs (c++): Number of total words: " << num_words << endl;
    cout << "clda-mgs (c++): Number of test words: " << num_test_words << endl;
    cout << "clda-mgs (c++): Number of topics: " << num_topics << endl;
    cout << "clda-mgs (c++): Vocabulary size: " << vocab_size << endl;
    cout << "clda-mgs (c++): alpha_h: " << alpha_h << endl;
    cout << "clda-mgs (c++): gamma_h: " << gamma_h << endl;
    cout << "clda-mgs (c++): eta_h: " << eta_h << endl;
    cout << "clda-mgs (c++): step_size: " << step_size << endl;
  }


  // The Gibbs sampling loop
  if (verbose > 0){
    cout << "clda-mgs (c++): Metropolis within Gibbs sampling..." << endl << endl;
  }
  ...
  for (iter = 0; iter < max_iter; iter++) { // for each Gibbs iteration
    ...
    if (verbose > 1) {
      cout << "clda-mgs (c++): gibbs iter# " << iter + 1;
    }
    ...
    for (d = 0; d < num_docs; d++) { // for each document
      ...
      if (save_flag && save_theta) {
        theta_samples.slice(ss_idx).col(d) = sample_dirichlet(num_topics, theta_counts.col(d) + gamma_h * pi_t.col(j));
      }
      else if (save_flag && ((iter + 1) == max_iter)) {
        theta_samples.slice(0).col(d) = sample_dirichlet(num_topics, theta_counts.col(d) + gamma_h * pi_t.col(j));
      }
      ...
      for (i = 0; i < n_jd; i++) { // for each word
        ...
        for (k = 0; k < num_topics; k++){ // for each topic
          ...
          prob(k) = (((theta_counts(k, d) + gamma_h * pi_t(k, j)) / doc_denom)
    ...
    if (iter > burn_in_pi) { // updates \pi's
      ...
      ////////////////////////////////////////////////////////////////////////////
      // One option is to use this epsilon as a sample from standard normal
      // arma::mat Epsilon = mvrnorm_arma(num_collections, mu, sigma);
      // For zero mean and unit standard deviation we can do this
      ////////////////////////////////////////////////////////////////////////////
      Epsilon = arma::randn(num_topics, num_collections); // to avoid negatives


      // computes gradients for the current \pi's
      arma::mat psi_t_sign = arma::sign(psi_t);
      psi_t_sign.elem( arma::find(psi_t_sign == 0) ).ones(); // change elements == 0 to 1

      arma::mat psi_t_abs = arma::abs(psi_t);
      arma::rowvec psi_t_abs_sum = arma::sum(psi_t_abs, 0); // sums elements in each column

      psi_grad = PSI_GRAD_INIT; // K x J matrix
      psi_grad /= psi_t_abs; // in-place element-wise division
      psi_grad -= 1.; // in-place element-wise substraction
      for (d = 0; d < num_docs; d++) { // for each document
        j = docs_cid(d); // document d's collection id
        psi_grad.col(j) += ((
          (gamma_h / (psi_t_abs_sum(j) * psi_t_abs_sum(j)))
          * arma::sum(
              psi_t_abs.col(j) % ( digamma_vec(gamma_h * psi_t_abs.col(j) / psi_t_abs_sum(j))
                                     - digamma_vec(gamma_h * psi_t_abs.col(j) / psi_t_abs_sum(j) + theta_counts.col(d)) )
            )
          )
                               -
                                 ((gamma_h / psi_t_abs_sum(j))
                                    * (digamma_vec(gamma_h * psi_t_abs.col(j) / psi_t_abs_sum(j))
                                         - digamma_vec(gamma_h * psi_t_abs.col(j) / psi_t_abs_sum(j) + theta_counts.col(d))
                                    )
                                 )
        );
      }
      psi_grad %= psi_t_sign; // applies sign
      ...
      // K x J matrix
      arma::mat psi_t_abs_grad = psi_t_abs % psi_grad; // natural gradient
      psi_t_abs_grad.each_row() /= arma::sqrt(arma::sum(psi_t_abs_grad % psi_t_abs_grad, 0));
      arma::mat mu_psi_t = psi_t + .5 * step_size_sqrd * (psi_t_abs_grad + psi_t_sign);
      // arma::mat mu_psi_t = psi_t + .5 * step_size_sqrd * (psi_t_abs % psi_grad + psi_t_sign);

      psi_prop = mu_psi_t + step_size * (arma::sqrt(psi_t_abs) % Epsilon);

      // /////////////////////////////////////////////////////////////////////////

      // computes gradients for the proposed \pi's
      // K x J matrix
      arma::mat psi_prop_sign = arma::sign(psi_prop);
      psi_prop_sign.elem( arma::find(psi_prop_sign == 0) ).ones(); // change elements == 0 to 1

      arma::mat psi_prop_abs = arma::abs(psi_prop);
      arma::rowvec psi_prop_abs_sum = arma::sum(psi_prop_abs, 0); // sums elements in each column

      psi_prop_grad = PSI_GRAD_INIT; // K x J matrix
      psi_prop_grad /= psi_prop_abs; // in-place element-wise division
      psi_prop_grad -= 1.;
      for (d = 0; d < num_docs; d++) { // for each document
        j = docs_cid(d); // document d's collection id
        psi_prop_grad.col(j) += ((( gamma_h / ( psi_prop_abs_sum(j) * psi_prop_abs_sum(j) ) )
                                    * arma::sum( psi_prop_abs.col(j)
                                                   % (digamma_vec(gamma_h * psi_prop_abs.col(j) / psi_prop_abs_sum(j))
                                                   - digamma_vec(gamma_h * psi_prop_abs.col(j) / psi_prop_abs_sum(j) + theta_counts.col(d))
                                                   )
                                    )
        )
                                   -
                                     (( gamma_h / psi_prop_abs_sum(j) )
                                        * (digamma_vec(gamma_h * psi_prop_abs.col(j) / psi_prop_abs_sum(j))
                                             - digamma_vec(gamma_h * psi_prop_abs.col(j) / psi_prop_abs_sum(j) + theta_counts.col(d))
                                        )
                                     )
      ...
      psi_prop_grad %= psi_prop_sign; // applies sign


      arma::mat psi_prop_abs_grad = psi_prop_abs % psi_prop_grad; // natural gradient
      psi_prop_abs_grad.each_row() /= arma::sqrt(arma::sum(psi_prop_abs_grad % psi_prop_abs_grad, 0));
      arma::mat mu_psi_prop = psi_prop + .5 * step_size_sqrd * (psi_prop_abs_grad + psi_prop_sign);
      // arma::mat mu_psi_prop = psi_prop + .5 * step_size_sqrd * (psi_prop_abs % psi_prop_grad + psi_prop_sign);

      // computes Metropolis Hastings ratio

      // log q-ratio
      arma::rowvec log_q_num = arma::sum(-0.5 * arma::log(psi_prop_abs), 0); // 1 x J vector
      arma::rowvec log_q_den = arma::sum(-0.5 * arma::log(psi_t_abs), 0); // 1 x J vector
      log_q_num += (-0.5 / step_size_sqrd) * arma::sum((psi_t - mu_psi_prop) % (psi_t - mu_psi_prop) / psi_prop_abs, 0);
      log_q_den += (-0.5 / step_size_sqrd) * arma::sum((psi_prop - mu_psi_t) % (psi_prop - mu_psi_t) / psi_t_abs, 0);
      arma::rowvec log_q_ratio_J = log_q_num - log_q_den; // 1 x J vector

      // log p-ratio
      arma::mat log_p_ratio = PSI_GRAD_INIT; // K x J matrix
      log_p_ratio %= (log(psi_prop_abs) - log(psi_t_abs)); 	// in-place element-wise multiplication
      log_p_ratio += (psi_t_abs - psi_prop_abs);
      for (d = 0; d < num_docs; d++) { // \sum_{d = 1}^D_j
        j = docs_cid(d); // document d's collection id
        arma::vec lpr = (
          log_gamma_vec(gamma_h * psi_prop_abs.col(j) + theta_counts.col(d)) -
            log_gamma_vec(gamma_h * psi_t_abs.col(j) + theta_counts.col(d)) +
            log_gamma_vec(gamma_h * psi_t_abs.col(j)) -
            log_gamma_vec(gamma_h * psi_prop_abs.col(j))
        );
        log_p_ratio.col(j) += lpr;
      }
      arma::rowvec log_p_ratio_J = arma::sum(log_p_ratio, 0); // 1 x J vector

      arma::rowvec log_a_J = log_p_ratio_J + log_q_ratio_J; // 1 x J vector

      // Metropolis update
      //
      for (j = 0; j < num_collections; j++){
        double a = exp(log_a_J(j));
        if (runif(1)(0) < min(a, 1.)){ // acceptance
          psi_t.col(j) = psi_prop.col(j); // updates on acceptance
          if (verbose > 1)
            cout << " A";
          else if (verbose > 0)
            cout << "+";
        ...
        if (verbose > 1) {
          printf("(%.3f, %.3f)", log_p_ratio_J(j), log_q_ratio_J(j));
        }
      }

      // Computes \pi_j's
      pi_t = arma::abs(psi_t);
      pi_t.each_row() /= arma::sum(pi_t, 0); // each column will sums to one
    ...
    if (save_flag && save_lp){
      double lppi = arma::accu(PSI_GRAD_INIT % log(pi_t));
      for (d = 0; d < num_docs; d++) { // \sum_{j=1}^J \sum_{k=1}^K
        j = docs_cid(d); // document d's collection id
  ...
  if (verbose > 0){
    cout << endl;
    cout << "clda-mgs (c++): Completed sampling." << endl;
  }
