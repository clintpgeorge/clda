...
List lda_cgs(
  ...
  cout << endl << endl;
  if (verbose > 1){
    cout << "lda-cgs (c++): Number of saved samples - "
         << valid_samples << endl;
  }


  if (verbose > 0){
    cout << "lda-cgs (c++): Initializes variables and count statistics....";
  }
  ...
  for (d = 0; d < num_docs; d++){
    ...
    if (test_word_share > 0 && d > num_train_docs){
      ...
      unsigned int num_train_words = (1. - test_word_share) * n_d;
  ...
  if (verbose > 1){
    cout << "lda-cgs (c++): Number of docs: " << num_docs << endl;
    cout << "lda-cgs (c++): Number of total words: " << num_words << endl;
    cout << "lda-cgs (c++): Number of test words: " << num_test_words << endl;
    cout << "lda-cgs (c++): Number of topics: " << num_topics << endl;
    cout << "lda-cgs (c++): Vocabulary size: " << vocab_size << endl;
    cout << "lda-cgs (c++): alpha_h: " << alpha_h << endl;
    cout << "lda-cgs (c++): eta_h: " << eta_h << endl;
  }
  ...
  if (verbose > 0){
    cout << "lda-cgs (c++): Collapsed Gibbs sampling..." << endl << endl;
  }
  ...
  for (iter = 0; iter < max_iter; iter++) { // for each Gibbs iteration
    ...
    if (verbose > 1) { cout << "lda-cgs (c++): gibbs iter# " << iter + 1; }
    ...
    for (d = 0; d < num_docs; d++) { // for each document
      ...
      doc_denom = doc_train_word_counts(d) - 1. + alpha_h * num_topics; // it's a constant for a term
    ...
    if (save_flag && (test_word_share > 0 && test_doc_share > 0)){
      ...
      for (d = 0; d < num_docs; d++) { // for each document
        ...
        theta_d_hat /= (doc_train_word_counts(d) + alpha_h * num_topics);
  ...
  if (verbose > 0){
    cout << endl;
    cout << "lda-cgs (c++): Completed sampling." << endl;
  }
